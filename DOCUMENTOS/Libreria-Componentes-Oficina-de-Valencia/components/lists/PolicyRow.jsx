...
export function PolicyRow({ policyType, insurer, policyNumber, policyStatus, divider = true, onClick }) {
    ...
    >
      <span style={{ display: "flex", alignItems: "baseline", gap: "var(--space-3)", minWidth: 0 }}>
        ...
        <span style={{
          ...
          textOverflow: "ellipsis",
          whiteSpace: "nowrap",
        }}>{[insurer, policyNumber].filter(Boolean).join(" · ")}</span>
      </span>
      <StatusBadge domain="policy" status={policyStatus} />
    </div>
  );
}
